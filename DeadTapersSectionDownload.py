...
def main():
    ...
    for dl_url in urls:
        ...
        if not args.no_download:
            ...
            print(os.getcwd()+'\\'+filenamefromurl(dl_url))
            download(dl_url, filenamefromurl(dl_url))
...
def extract_urls(soup):
    urls = []
    GDtable = soup.find('div', attrs={'class': 'field__items'})
    for row in GDtable.findAll("div", attrs={'class': 'field__item'}):
        urls.append(row.text)
    return urls
